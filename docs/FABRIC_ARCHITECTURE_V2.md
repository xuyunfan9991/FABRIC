...
新的三臂共卡候选资源身份统一冻结为 6 GiB planner target、2 GiB unmodeled reserve、4096 cell kernel cap 与每进程 8 GiB CUDA allocator ceiling；Full、ATAC、RBP 各有独立 YAML 和相同 condition 的 `ResourceProfile.<condition>.json`，不得用 Full profile 替代任一 ablation profile。新 profile 分别实跑同 cap 下高 EC-row 的 train batch（含 backward）和完整 validation planner 选出的 evaluation batch（`eval` + `no_grad`），覆盖两相各自的 workload extrema/quantiles；完整 epoch 时间仍由这些 selected batches 投影，不冒充实际遍历。三份配置的模型、objective、sampling、optimizer 与 scheduler 字段必须相同，资源打包差异不能进入 condition 对比。该数值策略只有在三份独占 condition profile 均通过 allocated/reserved 上限，且 ATAC+RBP 同卡 canary 覆盖两者 train/validation shape 并保留至少 2 GiB 物理 free memory 后，才构成共卡准入；只修改 target 字段或一次 `nvidia-smi` 截图均不构成完成证据。

...
JASPAR 中以 `A::B` 命名的 profile 表示 A/B heterodimer complex，不表示一个 motif 在两个可互换的单蛋白之间无法分辨。第一版没有 complex abundance、stoichiometry 或 joint-activity contract，因此这类 profile 必须以 `heterodimer_complex_unmodeled` 排除；不得把它转换为 `factor_equivalence_group`，也不得把 A、B 的单基因 activity 直接求和作为复合物活性。
...
模型训练前，必须对完整固定 route-feature design \([u^{base},u^{int}]\) 执行 zero-column、exact-duplicate 与 rank audit。若 base block 非满秩，必须先修正或删除语义重复的 base coding并更新 manifest；不得通过牺牲 interaction 掩盖 base 冗余。base 满秩后，按上述 canonical interaction-column 顺序只保留能增加完整 design rank 的 columns；对每个 field 记录 retained column indices \(S^{active}_{M,t}\)，并在 raw-cell coordinates 中定义 \(H^{active}_{M,t}=H^{support}_{M,t}[:,S^{active}_{M,t}]\)，各 field 的 indices 共同定义最终 \(m_M^{int}\)。这里的 signed rectangle entries 必须保持 \(-1/0/+1\)，并在整数/有理数域做 exact rank closure；模 2 相关不能证明实数模型中的线性相关。不得依靠 weight decay 在等价参数分解中任意选择一个解。raw support、candidate rectangles 与 comparator universe 不读取也不依赖 base reference；production rank audit 和 learned outputs 则只对 manifest 中冻结的 base coding 定义。
...
`data/DTU_result_sorted.xlsx` 明确定义为冻结的 DTU 基因先验；不再从 FABRIC train split 重算 DTU，也不为该工作簿增加额外 provenance 闸门。`data/DTU_score.R` 仅保留为该先验的历史生成记录：它以预先存在的 190-cell-type PSI/表达矩阵为输入，在 PSI、表达和 transcript-to-gene metadata 的共同 transcript 轴上要求至少两条 transcripts、至少两个 expressed cell types，并以 dominant-transcript switching 的 transcript-wise JS divergence 计算 score。工作簿与 matrix-matched GTF 的 28,002-gene transcript counts 完全一致；其中 `top_DTU_gene` 恰好等价于 `DTU_score >= 0.7`，但阈值赋值代码不在该 R 文件中。本轮 B2 variant 3 不新增或使用 high-DTU 二级标签，也不改变 gene admission；它只把 `G_fit.tsv` 中源自冻结工作簿的连续 `DTU_score` 转成全 G_fit cohort percentile，进入下一节明确定义的 bounded sensitivity loss。任何 `dtu_alpha > 0` 的结果都必须称为冻结先验加权 sensitivity，不能称为 train-only DTU；`dtu_alpha = 0` 时该先验不产生数值权重效应。
...
唯一真实数据运行 scope 是 `full_cohort`。它绑定 train-only admission 后的全部 17,600 个 \(G_{fit}\) genes，并同时保留 17,706 structural candidates、90,672 paths、217,933 cells 和 106 个 graph-only genes 的完整 audit；不得用历史 7,198-gene graph/EC 或 167,235-cell split。`data/processed/fabric_v2_real_dataset_atomic_introns_v1/prepared_dataset/` 已按 source identity `0f37946f14277f819b4dba34b637d2104bd1126a` 完整重建并通过严格验证：包含 17,600 个有序 G_fit shards、90,361 条 G_fit paths、495,448 条 G_fit edges、74,156,703 条 train/validation compatible rows 和 69,027,635 个 gene-cell instances。旧的 `data/processed/fabric_v2_real_dataset_v1/prepared_dataset/` 保留为语义修复前的历史制品，不得用于本次训练。106 个 graph-only genes 独立保留且不进入 likelihood fit；test compatible rows 为 0。
...
每个 completed epoch（该 epoch 的全部 gene updates、唯一完整 validation traversal、validation-NLL scheduler step 和 reporting-only ONT KL 均已完成）形成唯一恢复边界。runtime 必须以原子文件替换写入 `latest.pt`；它自包含当前 model/optimizer/scheduler、已完成 epoch、early-stopping counter、完整 history/monitor、gene-order RNG、Python/NumPy/Torch/CUDA RNG，以及当前 validation-NLL best 的 model/optimizer/scheduler state。`best.pt` 只在 validation NLL 严格改善时更新，训练正常结束后 `checkpoint.pt` 与 `best.pt` 均表示选中 best，而 `latest.pt` 保留最后 completed epoch 以供故障恢复。prepared build identity 与 compatible-artifact identity 分别使用其冻结制品目录名，不使用每次构建都会变化的时间戳；最后改变 `src/fabric` tree 的 Git commit 与 prepared artifact 的构建 commit 均单独进入恢复身份；自 2026-08-18 起（用户授权）两者不再要求相等——纯训练侧的 `src/fabric` 改动不强制字节级等价的数据重建，审计者可对恢复身份中记录的两个 commit 求 diff 以核验数据构建代码未被触碰。full-cohort build/训练仍要求 `src/fabric` tree clean。event-scan shard 只按文件存在不能证明完成：同一 output root 已有 shard 时明确失败并要求新的 build root。恢复只允许显式 `--resume-from <原 run-dir>/latest.pt` 并复用原 run-dir；seed、condition、resolved config、TrainingRunManifest、input/compatible-artifact identity、source commit、有序 G_fit、model shape 或 test-exposure 状态任一不一致均明确失败。恢复必须从 `completed_epoch+1` 开始，恢复 scheduler、early stopping、history 和全部 RNG 连续性；不得从 `best.pt` 分叉 history，不支持 gene 中途或 epoch 中途续跑，也不得因恢复读取 test。run-dir 使用进程互斥锁，禁止两个 trainer 同时改写同一恢复历史；若进程在 epoch 中途退出，磁盘上的上一份原子 `latest.pt` 仍是唯一合法恢复点。

截至 2026-08-15，旧的固定 attention cap 与 20.23-hour epoch 外推已经删除。历史 clean `ResourceProfile.json` 使用一张独占 RTX 4090、`full`、cap 512、完整 validation 的 workload 投影、两个 backed-shard traversals 和单 worker prefetch：17,600 个 capped train genes 全部规划为每 gene 一个 GPU batch，17,241 个具有 validation informative rows 的 genes 也各为一个 batch；29 个真实 train forward+NLL+backward 极值/分位 batches 的最大 allocation 为 10,330,077,696 bytes，未超过 shape estimator，投影为 2.994 hours/epoch。该历史 profile 没有实际遍历完整 validation，也没有构造 optimizer、调用 `optimizer.step`、执行 gradient clipping、保存 checkpoint、完成真实 full epoch 或读取 test prediction/metric；因此 2.994 hours 只是由 selected batches 外推且不含 17,600 次 AdamW update 与 clipping 开销的 train/validation 投影，不得冒充真实端到端 epoch wall time。learning rate 与 validation-driven scheduler 不改变已 profile 的 batch shape；optimizer-update wall time 只能由获准训练的实际日志补充。readiness 已在训练仍未授权时独立冻结；用户随后明确授权启动与提交/推送，当前 `configs/fabric_v2_full_cohort.yaml` 保持 `final_test_authorized=false`，仅将 `training_authorized=true` 与状态 `READY_TO_LAUNCH_FULL_COHORT_RUN` 打开。

新的三臂共卡候选资源身份统一冻结为 6 GiB planner target、2 GiB unmodeled reserve、4096 cell kernel cap 与每进程 8 GiB CUDA allocator ceiling；Full、ATAC、RBP 各有独立 YAML 和相同 condition 的 `ResourceProfile.<condition>.json`，不得用 Full profile 替代任一 ablation profile。新 profile 同时绑定 model config、input/compatibility identity 与 clean profiling source commit，并分别实跑同 cap 下高 EC-row 的 train batch（含 backward）和完整 validation planner 选出的 evaluation batch（`eval` + `no_grad`），覆盖两相各自的 workload extrema/quantiles；准入必须复核两相 batch 结构与顶层峰值汇总，不能只信任 profile 的状态标签。完整 epoch 时间仍由这些 selected batches 投影，不冒充实际遍历。三份配置的模型、objective、sampling、optimizer 与 scheduler 字段必须相同，资源打包差异不能进入 condition 对比。该数值策略只有在三份独占 condition profile 均通过 allocated/reserved 上限，且 ATAC+RBP 同卡 canary 覆盖两者 train/validation shape 并保留至少 2 GiB 物理 free memory 后，才构成共卡准入；只修改 target 字段或一次 `nvidia-smi` 截图均不构成完成证据。
...
33. `TrainingRunManifest` 恰好包含一个命令 seed 和一个 `full|atac|rbp` condition，并冻结 resolved learning rate、scheduler、`fixed_initial_epochs` 及其他 scheduler 参数、gradient clip norm、penalties、max epochs、early-stopping patience、per-gene cap、`optimizer_step_unit=train_positive_gene`、gene-microbatch accumulation、adaptive GPU target、可选 CUDA allocator ceiling、kernel cell cap 与 backed-shard prefetch identity；拒绝 embedded seed lists、condition lists、独立 `sampling_seed` 与内部 condition 名；相同 seed/condition/config/input 重建相同初始化、逐 epoch gene-cell 抽样和 gene update order，重复 seed 或 condition 由用户分别提交命令；
...
- [ ] **单任务资源估算已冻结**：每个拟启动命令必须基于相同 condition、model shape、gene-cell cap、gene-shape GPU policy 与 shard-prefetch policy 的 train/validation-only full-shape pilot，记录一次运行的 GPU-hours、峰值 GPU memory、host RAM、prepared/runtime storage、完整 validation 耗时投影、预计 epoch 数、early-stopping patience、wall-clock 和失败重跑预留。profile 若受到其他 GPU workload 竞争则 timing 不可冻结。profiling 不执行 `optimizer.step`、gradient clipping、不保存 checkpoint、不完成真实 full epoch，也不估算或授权隐藏的 multi-seed/multi-condition campaign；其耗时必须明确标为从 selected train/validation batches 外推且不含 optimizer-update/clipping 的投影。condition、model shape、gene-cell cap、batch/precision/prefetch policy 等 resource-relevant identity 变化后旧 profile 失效；仅 learning rate 或 validation-driven scheduler 改变不伪造新的 batch-shape profile。

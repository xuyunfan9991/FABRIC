...
唯一真实数据运行 scope 是 `full_cohort`。它绑定 train-only admission 后的全部 17,600 个 \(G_{fit}\) genes，并同时保留 17,706 structural candidates、90,672 paths、217,933 cells 和 106 个 graph-only genes 的完整 audit；不得用历史 7,198-gene graph/EC 或 167,235-cell split。现有 `data/processed/fabric_v2_real_dataset_v1/prepared_dataset/` 是本轮代码语义修复前生成的历史制品；其已报告规模为 17,600 个有序 G_fit shards、90,361 条 G_fit paths、493,310 条 G_fit edges、74,156,703 条 train/validation compatible rows 和 69,027,635 个 gene-cell instances，不能在未重建并通过新 source identity 检查时直接训练。106 个 graph-only genes 独立保留且不进入 likelihood fit；test compatible rows 为 0。
